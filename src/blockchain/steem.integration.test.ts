...
describe("test/integration/steem.spec.ts", () => {
    describe("account_history_api.get_account_history", function() {
        this.timeout(15000);
        const steem: steemJs.api.Steem = new steemJs.api.Steem({ url: testsConfig.defaultSteemApi });
    ...
    describe("BlockchainConfig", () => {
        it("contains correct values", async () => {
            ...
            for (const v of BlockchainConfig.configValueAssertionArray as Array<[string, any]>) {
                const variable = v as [string, any];
