...
export class SteemOperationNumber {
    ...
    public isGreaterThan(tn: SteemOperationNumber): boolean {
        if (this.blockNum > tn.blockNum) {
            return true;
        } else if (this.blockNum == tn.blockNum && this.transactionNum > tn.transactionNum) {
            return true;
        } else if (this.blockNum == tn.blockNum && this.transactionNum == tn.transactionNum) {
            return this.operationNum > tn.operationNum;
        } else {
            return false;
        }
    }
     ...
     * Issue: https://github.com/steemit/steem/issues/2272 It is solved, but in api.steemit.com endpoint
    ...
    public isLesserThan_solveOpInTrxBug(tn: SteemOperationNumber): boolean {
        if (this.blockNum < tn.blockNum) {
            return true;
        } else if (this.blockNum == tn.blockNum && this.transactionNum <= tn.transactionNum - 1) {
            return true;
        } else {
            return false;
        }
    ...
    public isEqual(tn: SteemOperationNumber): boolean {
        return (
            this.blockNum == tn.blockNum &&
            this.transactionNum == tn.transactionNum &&
            this.operationNum == tn.operationNum
        );
    }
